...
class AugmentedDataset(Dataset):
    ...
    def __getitem__(self, idx: int
        ...
        # Implement steps: get_finger_points -> KeypointsOnImage -> self.aug_pipeline -> get_center_angle 
        orig_center_point = data_torch['center_point'].numpy()
        orig_angle = data_torch['angle'].numpy()
        left_coord, right_coord = get_finger_points(orig_center_point, orig_angle)

        keypoints = ia.KeypointsOnImage([
            ia.Keypoint(x=left_coord[0], y=left_coord[1]),
            ia.Keypoint(x=right_coord[0], y=right_coord[1])
        ], shape=data_torch['rgb'].shape)
        #print("before aug: ", keypoints)

        image_aug, kps_aug = self.aug_pipeline(image=data_torch['rgb'].numpy(), keypoints=keypoints)
        #print("after aug: ", kps_aug)

        new_center_point, new_angle = get_center_angle(kps_aug.keypoints[0].xy, kps_aug.keypoints[1].xy)

        data_torch['rgb'] = torch.from_numpy(image_aug)
        data_torch['center_point'] = torch.from_numpy(new_center_point)
        data_torch['angle'] = torch.from_numpy(np.array([new_angle]))
